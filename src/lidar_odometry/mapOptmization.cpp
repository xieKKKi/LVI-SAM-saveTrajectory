...
public:
    ...
    void visualizeGlobalMapThread()
    {
        ...
        if(saveTXT){
            ...
            // 1.create directory and remove old files, 删除文件夹再重建!!!
            saveTXTDirectory = std::getenv("HOME") + saveTXTDirectory;
            // system((std::string("exec rm -r ") + saveTXTDirectory).c_str());
            system((std::string("mkdir ") + saveTXTDirectory).c_str());    
            ofstream foutC;
            foutC.open(saveTXTDirectory + "lidar_trajectory.txt");
    ...
    void performLoopClosure(const std_msgs::Float64MultiArray& loopMsg) // loopMsg保存的是时间戳
    {
        ...
        {
            loopFindNearKeyframes(copy_cloudKeyPoses6D, cureKeyframeCloud, key_cur, 0);  // 是0？？？
